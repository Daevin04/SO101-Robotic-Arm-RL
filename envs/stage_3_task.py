...
Stage 3 (THIS): Ultra-Precise Positioning - EXTREME tolerances (±1.0-1.2cm) + infinite hold

================================================================================
TASK OBJECTIVE
================================================================================
Force ROCK-SOLID positioning by rewarding SUSTAINED precision, not brief contact.

PRIMARY GOALS:
    1. **Complete Stage 1 first**: Base must be aligned with cube
    2. **Achieve ultra-precise positioning**: Get claw tip within ±1.0-1.2cm
    3. **MAINTAIN position for entire episode**: Rewarded every step positioned
    4. **Zero cube pushing**: Massive penalties prevent exploit

SPECIFIC REQUIREMENTS:
    - Stage 1 gate: Base aligned within 15° (same as Stage 2)
    - Positioning target (MUCH TIGHTER than Stage 2):
        * Y (depth): Within ±1.0cm of cube (was ±2cm in Stage 2)
        * X (side): Within ±1.2cm offset (was ±3cm in Stage 2)
        * Z (height): Within ±1.0cm of cube (was ±2cm in Stage 2)
...
Success Criteria: Maximize time spent in precise position (no termination)

================================================================================
REWARD STRUCTURE (ANTI-EXPLOIT DESIGN)
================================================================================
...
STAGE 2 REWARDS (only after Stage 1 complete):
    +10 * exp(-3 * dist)      Distance reward (steeper falloff for precision)
                               - 0cm: +10 reward
                               - 5cm: +2.2 reward
                               - 10cm: +0.5 reward

    +50 EVERY STEP            Continuous reward for maintaining precise position:
                               - Y within ±1.0cm of cube
                               - X within ±1.2cm (cube between jaws)
                               - Z within ±1.0cm of cube
                               - Accumulates: 150 steps = 7,500 points!
...
Total possible reward per episode:
    - Perfect positioning all 150 steps: ~9,000 points
    - Brief positioning (10 steps): ~500-1000 points
    - Pushing cube: Large negative rewards

Why infinite hold design?
    ✅ Prevents "lucky brief positioning" exploit
    ✅ Forces learning STABLE positioning, not just hitting target
    ✅ Massive cube penalty prevents "push target to claw" exploit
    ✅ Tighter tolerances prevent boundary exploitation
    ✅ Reward proportional to consistency, not just achievement
...
class Stage3Task(SO101BaseEnv):
    """Stage 3: Ultra-Precise Positioning with Infinite Hold (Anti-Exploit Design)"""

    # Stage 1 gate criteria (same as Stage 2)
    ALIGNMENT_THRESHOLD = 0.26  # Within 15 degrees = Stage 1 complete

    # Stage 3 positioning criteria - ULTRA-TIGHT (anti-exploit)
    MAX_Y_DIFF = 0.010           # 1.0cm - depth (was 1.5cm, now 33% tighter)
    MAX_X_DIFF = 0.012           # 1.2cm - side offset (was 2cm, now 40% tighter)
    MAX_Z_DIFF = 0.010           # 1.0cm - height (was 1.5cm, now 33% tighter)
    CLAW_OFFSET_X = 0.025        # 2.5cm offset from cube center

    # Infinite hold reward structure
    POSITION_HOLD_REWARD = 50.0  # Reward EVERY step for maintaining position
    DISTANCE_REWARD_SCALE = 10.0 # Scale for distance-based reward
    DISTANCE_REWARD_FALLOFF = 3.0 # Steeper falloff (was 2.0)
    ...
    def step(self, action):
        ...
        # Check if precisely positioned (ultra-tight tolerances)
        ...
        # NO EARLY TERMINATION - infinite hold design
        # Agent must maintain position for entire episode to maximize reward
        terminated = False
    ...
    def _compute_reward(self):
        """STAGE 3: Infinite hold design with extreme anti-exploit measures.

        Stage 1 gate → Continuous positioning rewards + massive push penalties
        """
        # Check Stage 1 gate
        if not self._stage1_complete:
            return 0.0

        # Stage 1 passed! Give positioning rewards
        distance = self._get_distance_to_target()

        # Distance reward with steeper falloff for precision
        distance_reward = self.DISTANCE_REWARD_SCALE * np.exp(
            -self.DISTANCE_REWARD_FALLOFF * distance
        )

        # MAIN REWARD: Continuous reward for HOLDING precise position
        # This is given EVERY step, not just once
        # Incentivizes maintaining position for entire episode
        position_reward = self.POSITION_HOLD_REWARD if self._is_positioned() else 0.0
    ...
    def _is_positioned(self):
        """Check if claw tip is positioned correctly (ultra-tight tolerances)."""
        claw_tip_pos = self.data.site_xpos[self.left_fingertip_id]
        cube_pos = self.data.site_xpos[self.object_site_id]
        target_pos = cube_pos.copy()
        target_pos[0] += self.CLAW_OFFSET_X
    ...
    def reset(self, seed=None, options=None):
        ...
        # Hide goal marker (not used in Stage 3)
